...
static void D_Display (void)
{
    ...
    { // In a level
        ...
        /* The status bar is meaningless on a 13x8 matrix, and the Squashware
         * IWAD strips the STGANUM* status-bar number graphics, so ST_Drawer
         * dereferences missing patches and crashes. Skip it for charlieplex.
         */
...
static void D_DoomMainSetup(void)
{
    ...
    {
#if defined(CONFIG_DOOM_CHARLIEPLEX)
        /* On the 13x8 matrix the title screen and demo playback are static /
         * unreadable, and we drive input over the SPI bridge. Boot straight
         * into a live game on E1M1 so there is an actual moving first-person
         * view to downscale, and so WASD/fire input has something to control.
         */
