...
ZTEST(charlieplex, test_write_out_of_bounds)
{
	...
	zassert_not_ok(display_write(disp, MATRIX_W, 0, &desc, buf),
		       "out-of-bounds x accepted");
	zassert_not_ok(display_write(disp, 0, MATRIX_H, &desc, buf),
		       "out-of-bounds y accepted");
}
