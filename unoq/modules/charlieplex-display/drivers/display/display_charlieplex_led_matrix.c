 ...
 * scans the matrix fast enough for persistence of vision.
 *
...
LOG_MODULE_REGISTER(charlieplex_led_matrix, CONFIG_DISPLAY_LOG_LEVEL);
...
static void cplx_light(const struct device *dev, uint16_t pixel)
{
	...
	/* Drive low first then high to avoid a transient on the wrong pair. */
	gpio_pin_configure_dt(&cfg->gpios[lo], GPIO_OUTPUT_INACTIVE);
	gpio_pin_configure_dt(&cfg->gpios[hi], GPIO_OUTPUT_ACTIVE);
	data->cur_low = lo;
	data->cur_high = hi;
}
...
static int cplx_start_timer(const struct device *dev)
{
	...
	if (ticks == 0) {
		...
		LOG_WRN("counter freq %u Hz too low for %u Hz scan; clamping",
			freq, tick_hz);
		ticks = 1;
	}
...
static void cplx_get_capabilities(const struct device *dev,
...
static int cplx_set_pixel_format(const struct device *dev,
...
#define CPLX_GPIO_SPEC(node_id, prop, idx) \
...
#define CPLX_INIT(inst)                                                        \
	...
	static const uint16_t cplx_pixel_map_##inst[] =                        \
		DT_INST_PROP(inst, pixel_pairs);                                 \
	...
	static const struct cplx_config cplx_config_##inst = {                 \
		...
		.grayscale_bits = DT_INST_PROP_OR(inst, grayscale_bits, 1),    \
		.refresh_hz = DT_INST_PROP(inst, refresh_frequency),           \
	};                                                                     \
	static struct cplx_data cplx_data_##inst = {                           \
		.framebuf = cplx_framebuf_##inst,                              \
	};                                                                     \
	DEVICE_DT_INST_DEFINE(inst, cplx_init, NULL,                           \
			      &cplx_data_##inst, &cplx_config_##inst,          \
			      POST_KERNEL,                                     \
			      CONFIG_CHARLIEPLEX_LED_MATRIX_INIT_PRIORITY,     \
			      &cplx_api);
